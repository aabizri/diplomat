...
internal unsafe struct DiplomatSlice<T> where T : unmanaged
{
    ...
    /// <summary>
    /// Creates a memory manager that can deliver a Memory<<typeparamref name="T"/>>
    /// </summary>
    /// <returns>An UnmanagedMemoryManager over <typeparamref name="T"/></returns>
    public readonly UnmanagedMemoryManager<T> GetMemoryManager() => new(this);
}
